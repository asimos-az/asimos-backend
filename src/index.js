...
async function notifyNearbySeekers(job) {
  if (!job || !job.location || !job.location.lat || !job.location.lng) return;

  // Max radius to notify nearby seekers (e.g. 5km)
  // Or use job.notifyRadiusM if provided, otherwise default to 5000m
  const radiusM = job.notifyRadiusM || 5000;
  const lat = job.location.lat;
  const lng = job.location.lng;

  // 1. Find seekers within range
  // Since we don't have PostGIS, we fetch all seekers with location and filter in JS (inefficient for large scale, but fine for now)
  // Optimisation: Filter by bounding box in SQL if possible, or just limit to recent active users
  ...
  const validSeekers = seekers.filter(s => {
    ...
    const dist = haversineDistanceM(lat, lng, s.location.lat, s.location.lng);
    return dist <= radiusM;
  });
  ...
  const title = "Yeni İş Elanı!";
  const body = `Yaxınlığınızda yeni iş: ${job.title} (${job.wage ? job.wage + " AZN" : "Razılaşma"})`;

  const notifications = [];
  const pushMessages = [];

  for (const s of validSeekers) {
    // In-app notification
    notifications.push({
      user_id: s.id,
      title,
    ...
    // Push notification (if token exists in profile)
  ...
  // Also check push_tokens table for these users
  ...
  if (extraTokens) {
    for (const t of extraTokens) {
      // Avoid duplicates if profile token is same
      const alreadyAdded = pushMessages.find(p => p.to === t.expo_push_token);
      if (!alreadyAdded) {
        pushMessages.push({
          to: t.expo_push_token,
          title,
