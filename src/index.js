...
app.get("/companies", async (req, res) => {
  try {
    ...
    for (const job of jobsRes.data || []) {
      ...
      if (!name) continue;
      const knownId = companyIdByName.get(normalizeName(name));
      const key = knownId || `company:${Buffer.from(name, "utf8").toString("base64url")}`;
...
app.get("/companies/:id", async (req, res) => {
  try {
    ...
    if (companyId.startsWith("company:")) {
      try {
        requestedCompanyName = Buffer.from(companyId.slice("company:".length), "base64url").toString("utf8").trim();
      } catch {
        return res.status(400).json({ error: "Şirkət linki yanlışdır" });
      }
    } else {
      ...
      profile = data;
      requestedCompanyName = String(profile?.company_name || "").trim();
    }

    if (!requestedCompanyName) return res.status(404).json({ error: "Şirkət tapılmadı" });
    ...
    const jobs = (rows || []).filter((job) => {
      if (normalizeName(job.company_name) !== normalizeName(requestedCompanyName)) return false;
      const expiresMs = job.expires_at ? new Date(job.expires_at).getTime() : null;
      const createdMs = job.created_at ? new Date(job.created_at).getTime() : null;
      ...
      companyName: job.company_name || requestedCompanyName,
      company_name: job.company_name || requestedCompanyName,
    ...
    return res.json({
      company: {
        id: profile?.id || companyId,
        companyName: profile?.company_name || requestedCompanyName,
        logoUrl: profile?.logo_url || rows?.[0]?.company_logo_url || rows?.[0]?.image_url || "",
        category: profile?.category || rows?.[0]?.category || "Müxtəlif sahələr",
        location: profile?.location || (rows?.[0]?.location_address ? { address: rows[0].location_address } : null),
        verified: profile ? !["blocked", "rejected", "deleted"].includes(String(profile.status || "").toLowerCase()) : false,
  ...
  try {
    ...
    let items = (data || []).map((r) => {
      ...
      const job = {
        ...
        wage: r.wage,
        views: Number(r.views || 0),
        whatsapp: r.whatsapp ?? r.contact_whatsapp ?? r.contact_phone ?? r.phone ?? null,
        phone: r.contact_phone ?? r.phone ?? r.whatsapp ?? null,
        email: r.contact_email ?? null,
        contactEmail: r.contact_email ?? null,
        contact_email: r.contact_email ?? null,
        link: r.contact_link ?? r.ats_link ?? null,
        atsLink: r.ats_link ?? r.contact_link ?? null,
        ats_link: r.ats_link ?? r.contact_link ?? null,
...
app.get("/jobs/:id", optionalAuth, async (req, res) => {
  try {
    ...
    const job = {
      ...
      whatsapp: data.whatsapp ?? data.contact_whatsapp ?? data.contact_phone ?? data.phone ?? null,
      phone: data.contact_phone ?? data.phone ?? data.whatsapp ?? null,
      email: data.contact_email ?? null,
      contactEmail: data.contact_email ?? null,
      contact_email: data.contact_email ?? null,
      link: data.contact_link ?? data.ats_link ?? null,
...
app.post("/jobs", requireAuth, async (req, res) => {
  try {
    ...
    const job = {
      ...
      category: data.category,
      description: data.description,
      wage: data.wage, whatsapp: data.whatsapp ?? data.contact_whatsapp ?? data.contact_phone ?? data.phone ?? null,
      phone: data.contact_phone ?? data.phone ?? data.whatsapp ?? null,
      email: data.contact_email ?? null,
      contactEmail: data.contact_email ?? null,
      contact_email: data.contact_email ?? null,
...
app.patch("/jobs/:id", requireAuth, async (req, res) => {
  try {
    ...
    const allowed = {
      ...
      whatsapp: (body.whatsapp !== undefined || body.contact_whatsapp !== undefined || body.contactWhatsapp !== undefined) ? (body.whatsapp || body.contact_whatsapp || body.contactWhatsapp || null) : undefined,
    ...
    const job = {
      ...
      wage: updated.wage,
      whatsapp: updated.whatsapp ?? updated.contact_whatsapp ?? updated.contact_phone ?? updated.phone ?? null,
      phone: updated.contact_phone ?? updated.phone ?? updated.whatsapp ?? null,
...
app.patch("/jobs/:id/close", requireAuth, async (req, res) => {
  try {
    ...
    return res.json({
      ...
      category: updated.category,
      description: updated.description,
      wage: updated.wage, whatsapp: updated.whatsapp ?? updated.contact_whatsapp ?? updated.contact_phone ?? updated.phone ?? null,
      phone: updated.contact_phone ?? updated.phone ?? updated.whatsapp ?? null,
      email: updated.contact_email ?? null,
      contactEmail: updated.contact_email ?? null,
      contact_email: updated.contact_email ?? null,
